...
export class AppComponent {
  ...
  remove(id: number) {
    const itemIndex = this.cartItems.findIndex((obj) => obj.ISBN === id);

    const item = this.cartItems[itemIndex];

    this.total -= item.quantity * item.price.value;
    this.total= parseFloat((this.total).toFixed(2))
  ...
  removeItem(ISBN: number) {
    ...
    if (this.cartItems[itemIndex].quantity > 1) {
      ...
      this.total -= this.cartItems[itemIndex].price.value;
      this.total= parseFloat((this.total).toFixed(2))
    } else {
      this.remove(this.cartItems[itemIndex].ISBN);
    }
  }
...
searchValue: string='';
ValueToBeSearched(valueSearch:string){
